...
export const Auth: React.FC<Props> = ({ onLogin, logActivity }) => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-blue-50 px-4 font-sans py-10">
      ...
      <div className="bg-white/80 backdrop-blur-xl p-10 rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.05)] w-full max-w-md border border-white relative overflow-hidden">
        <div className="absolute top-0 right-0 w-40 h-40 bg-blue-100/50 rounded-bl-full -mr-16 -mt-16 z-0 blur-3xl"></div>
        <div className="absolute bottom-0 left-0 w-40 h-40 bg-indigo-100/50 rounded-tr-full -ml-16 -mb-16 z-0 blur-3xl"></div>
        
        <button onClick={() => setShowGuide(true)} className="absolute top-6 left-6 z-20 text-slate-400 hover:text-blue-600 transition-colors">
            <HelpCircle size={22} />
        </button>

        <div className="text-center mb-10 relative z-10">
          <div className="w-24 h-24 bg-white rounded-3xl flex items-center justify-center mx-auto mb-6 shadow-xl ring-8 ring-slate-50/50 p-1 overflow-hidden transform -rotate-3 hover:rotate-0 transition-transform duration-500">
              {settings?.appLogo ? (
                  <img src={settings.appLogo} alt="App Logo" className="w-full h-full object-cover rounded-2xl" />
              ) : (
          ...
          <p className="text-slate-500 font-medium tracking-wide text-xs uppercase italic">Premium Learning Platform</p>
        </div>
